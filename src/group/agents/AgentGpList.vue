<template>
    <div class="agent_gp_list" data-name="agent_gp_list">
        ...
        <template v-else>
            ...
            <div class="btns flex_item">
                <button class="btn_common" @click="memberStatusChange(2)">拒绝</button>
                <button class="btn_common" @click="memberStatusChange(1)">通过</button>
            </div>
        </template>
    </div>
</template>
<script>
import { agentGpJoinList,agentGpJoinAdd } from '../../../api/sendRequest';
...
export default {
    ...
    methods:{
        async getList(){
            let res = await agentGpJoinList(this.user_info.yyid,this.user_info.user_token,this.$route.query.group_yyid);
            if(res.errcode != 0){
        ...
        async memberStatusChange(type){
            if(this.check_list.length === 0){
                this.$vux.toast.text('请选择人员','top');
                return
            };
            let res = await agentGpJoinAdd(this.user_info.yyid,this.user_info.user_token,this.$route.query.group_yyid,this.check_list.toString(),type);
            let _this = this;
            if(res.errcode == 0){
