<template>
	<div class="select_hos" data-name="page_certify_sel_hos">
		...
		<transition name="show_cover">
			<div class="hos_detail" v-show="hos_detail"></div>
		</transition>
		<section class="tab_item">
			<transition name = 'showlist'>
				<section class="show_hos_info" v-show="hos_detail">
					<section class="hos_info bgcw">
						<div class="title">
							...
							<div class="sign">
								...
								<span v-else class="yellow">已进药</span>
							</div>
						</div>
						<div class="base_info">
					...
					<div class="close">
						<i class="iconfont" @click="closeCover">&#xe615;</i>
					</div>
				</section>
			</transition>
		</section>
		<!-- 已选医院列表 -->
		<transition name="show_cover">
			<div class="selected_cover" v-show="selected_list" @click="selectedStatu()"></div>
		</transition>
		<section class="tab_item selected_list bgcw">
			<transition name = 'show_list'>
				<section class="search_list" v-show="selected_list">
					<div class="hos_item flex_item bd_bt" v-for="(item,index) in cart_list">
						<div class="hos_item_l">
							<div class="hos_name">{{item.hos_name}}</div>
							<div class="hos_sign">
								<span v-show="item.hos_level">{{item.hos_level}}</span>
							</div>
						</div>
						<div class="select_status">
							<i class="iconfont" @click="delHos(item.hos_id)">&#xe615;</i>
						</div>
					</div>
				</section>
			</transition>
		...
		<mu-paper :z-depth="1" class="demo-loadmore-wrap search_list">
			<mu-container ref="container" class="demo-loadmore-content">
				<mu-load-more :loading="loading" :loaded-all="loaded_all" @load="getHosList" >
					<template>
						<div class="hos_item flex_item" v-for="(item,index) in hos_list" :key="index">
							<div class="hos_item_l" @click="popHosInfo(item)">
								<div class="hos_name">{{item.hospital_name}}</div>
								<div class="hos_sign">
									<span v-show="item.level">{{item.level}}</span>
								</div>
							</div>
							<div class="select_status" @click="addSelect(item.hospital_yyid,item.hospital_name,item.level)">
								<i v-if="yyid_list.indexOf(item.hospital_yyid) == -1" class="iconfont">&#xe668;</i>
		...
		<div class="hos_cart bgcw flex_item">
			<a href="javascript:;" class="hos_num flex_item bgcw" @click="selectedStatu()">
				<span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;已选医院</span>
				<div class="selected_num">
					<p class="wave" :class="{ wave_circle:wave_status}"></p>
					<span class="">
						{{yyid_list.length}}
					</span>
				</div>
			...
			<a href="javascript:;" class="hos_sub" @click="backLast()">确认</a>
		</div>
	</div>
</template>
<script>
	import { Search } from 'vux'
	import {hospitalList,hospitalDetail} from '../../api/sendRequest'
	...
	export default {
		...
		methods:{
			...
			delHos(id){
				// 删除已选择的医院
				if(this.yyid_list.length == 1){
					this.selected_list = !this.selected_list;
				}
				this.DELETE_CART_LIST({'hos_id':id})
			},
			popHosInfo(item){
				// 点击弹出医院信息
				this.hos_detail = true;
				this.getHosDetail(item.hospital_yyid);
			},
			closeCover(){
				this.hos_detail = !this.hos_detail;
			},
			selectedStatu(){
