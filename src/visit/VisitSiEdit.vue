<template>
    <div class="visit_edit">
        ...
        <div class="v_item visit_hos bd_bt">
            ...
            <router-link :to="{path:'/visitHos',query:{type:1}}">{{hos_id?(visit_check_in.v_hos == "请选择"?'请选择':visit_check_in.v_hos):'请选择'}}</router-link>
            <i class="iconfont r_icon">&#xe661;</i>
        </div>
        <div class="v_item bd_bt">
        ...
        <div class="v_item bd_bt gps">
            <div class="flex_item">
                ...
                <div class="position" @click="getLocal(3)"> <i class="iconfont">&#xe79e;</i>{{visit_check_in.v_pos?'重新定位':"获取地理位置"}}</div>
            </div>
            <div class="flex_item">
...
<script>
import { getWxConfig, visitGetToken, visitUpload, visitGeo, checkGeo, visitConfig, visitCheckIn } from '../../api/sendRequest';
...
export default {
    ...
    mounted(){
        ...
        this.hos_id = getSessionStore('v_hos_id')?getSessionStore('v_hos_id'):'';
        let type = 1;
        // console.log(obj)
        if(obj){
    ...
    methods:{
            ...
            wx.ready(function(){
                wx.getLocation({
                    ...
                    success: async function (res) {
                        ...
                        // 检查是否需要调用位置检验
                        if(visit_check_in.v_way == 1 || visit_check_in.v_way == 2){
                            let res3 = await checkGeo(_this.user_info.yyid,_this.user_info.user_token,getSessionStore('v_hos_id'),_this.lat,_this.lng);
                            if(res3.errcode != 0){
        ...
        uploadError(err) {
            this.$vux.toast.text('图片已存或图片数据错误')
        },
        afterRead(file) {
            // 此时可以自行将文件上传至服务器
        ...
        async checkIn(){
            let visit_check_in = this.visit_check_in;
            let hos_id = visit_check_in.v_hos != '请选择'?getSessionStore('v_hos_id'):0;
