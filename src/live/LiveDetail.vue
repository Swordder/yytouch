...
<script>
	...
	import { liveDetail,favStatus,commentsList,sendComment,delComment,viewCount } from '../../api/sendRequest.js'
	import { getSessionStore,setStore } from '../../config/util.js'
	...
	export default {
		...
		mounted() {
			...
			if(this.user_info && this.user_info.yyid){
				viewCount(this.user_info.yyid,this.$route.query.video_yyid,this.share_yyid,7,window.sessionStorage.getItem('singleMark'))
			} else {
				viewCount('',this.$route.query.video_yyid,this.share_yyid,7,window.sessionStorage.getItem('singleMark'))
			}
		},
		methods:{
			idStatus(){
			...
			async shareInfo(){
				let res = await axiosFn('/youyao/share/info',{
					'page_name':'playback_detail',
					'video_yyid':this.$route.query.video_yyid
				},'POST');
				wxShare(res.data.title,res.data.sub_title,res.data.image,this.$route.query.video_yyid,7,this.$route.query);	
			},
			async getVideoDetail(){
				let res = await liveDetail(this.$route.query.video_yyid);
