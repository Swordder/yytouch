<template>
	<div class="live" data-name="page_live">
		...
		<div class="pub_class">
			<div class="com_hd flex_item">
				<h4>{{$t('pub_course')}}</h4>
				<router-link :to="{path:'/liveList',query:{type:1}}">{{$t('all_course')}}</router-link>
			</div>
			<div class="pub_list flex_item">
				<div class="pub_item" v-for="(item,index) in pub_list" :key="index">
					<router-link :to="{path:'/liveDetail',query:{video_yyid:item.yyid}}">
						<div class="pub_pic">
		...
		<div class="pub_class lecture">
			<div class="com_hd flex_item">
				<h4>{{$t('pro_lecture')}}</h4>
				<router-link :to="{path:'/liveList',query:{type:2}}">{{$t('all_lectures')}}</router-link>
			</div>
			<div class="pub_list">
				<div class="pub_item bd_bt" v-for="(item,index) in lecture_list" :key="index">
					<router-link class="flex_item" :to="{path:'/liveDetail',query:{video_yyid:item.yyid}}">
						...
						<div class="lecture_con flex_item">
							...
							<div class="pub_data flex_item">
								<div class="pub_time">
									{{item.start_time.split(' ')[0].split('-')[1]}}/{{item.start_time.split(' ')[0].split('-')[2]}}
									{{item.start_time.split(' ')[1].split(':')[1]}}:{{item.start_time.split(' ')[1].split(':')[1]}}
								</div>
								<div class="live_view flex_item">
									<p>
										<i class="iconfont">&#xe621;</i>
...
<script>
	...
	export default{
		...
		mounted() {
			this.userStatus();
			this.getTotalData();
			wxShare('优药直播','为了更好的推动临床技术的发展，造福患者，优药直播将定期邀请各位专家进行专业信息交流，提高手术技巧、了解最新进展、讨论热点问题。','http://wx.youyao99.com/images/icon.jpg','9D8003C158248FEA68767E86517B505B',8,this.$route.query);
		},
		methods:{
			userStatus(){
			...
			async getTotalData(){
				...
				let res1 = liveBanner(this.yyid,this.token,'');
				let res2 = liveAdvance(this.yyid,this.token,0);
				let res3 = liveVideoList(this.yyid,this.token,1,1);
				let res4 = liveVideoList(this.yyid,this.token,1,2);
				let res = await Promise.all([res1,res2,res3,res4]);

				// this.$vux.loading.hide();
				this.banner_list = res[0].data;
				this.advance_list = res[1].data;
				this.pub_list = res[2].data.video_list;
				this.lecture_list = res[3].data.video_list;
			},
